...
private[ml] abstract class Tree extends Serializable {
	...
	/**
	  ...
	  */
	def query(v: Vector): Iterable[(RowWithVector, Double)] = query(new VectorWithNorm(v))
	def query(v: VectorWithNorm): Iterable[(RowWithVector, Double)] = query(new KNNCandidates(v)).toIterable
...
case class SpillTree(
	...
	override def query(candidates: KNNCandidates): KNNCandidates = {
		if (size <= 1 - candidates.candidates.size) {
	...
	def build(data: IndexedSeq[RowWithVector], leafSize: Int = 1, tau: Double, seed: Long = 0L): Tree = {
		...
		if (size == 0) {
			...
			if (leftPivot == randomPivot) {
				...
				val pivot = new VectorWithNorm(Vectors.fromBreeze((leftPivot.vector.asBreeze + rightPivot.vector.asBreeze) / 2.0))
				val radius = math.sqrt(data.map(v => pivot.fastSquaredDistance(v.vector)).max)
				val dataWithDistance = data.map(v => (v, leftPivot.fastDistance(v.vector), rightPivot.fastDistance(v.vector)))
...
/**
  ...
  */
private[knn]
class KNNCandidates(val queryVector: VectorWithNorm) extends Serializable {
	...
	def insert(v: RowWithVector, d: Double): Unit = {
		while (candidates.nonEmpty) candidates.dequeue()
	...
	def notFull: Boolean = candidates.isEmpty
