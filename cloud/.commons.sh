...
_ICC_COMMONS_AWS_UID=`"$_ICC_COMMONS_AWS" sts get-caller-identity --query UserId` 2> /dev/null || die "AWS authentication failed. Is the access key correct?"

_ICC_COMMONS_NAME="francescozoccheddu-bdp-image-classifier"
_ICC_COMMONS_CLUSTER_NAME="$_ICC_COMMONS_NAME"
_ICC_COMMONS_KEY_NAME="$_ICC_COMMONS_NAME"
_ICC_COMMONS_KEY_FILE="$SDIR/.key.pem"
_ICC_COMMONS_BUCKET_NAME="$_ICC_COMMONS_NAME-$_ICC_COMMONS_AWS_UID-b"
...
function create_cluster {
	...
	"$_ICC_COMMONS_AWS" emr create-default-roles >& /dev/null || die "Failed to create EMR default roles." 
	local OPTS=""
...
function authorize_ssh {
	...
	_ICC_COMMONS_MY_IP=`curl -s http://checkip.amazonaws.com/` 2> /dev/null || die "Failed to get local machine IP."
	_ICC_COMMONS_SECURITY_GROUP_ID=`"$_ICC_COMMONS_AWS" emr describe-cluster --cluster-id "$_ICC_COMMONS_CLUSTER_ID" --query Cluster.Ec2InstanceAttributes.EmrManagedMasterSecurityGroup` 2> /dev/null || die "Failed to get the security group."
...
function terminate_cluster {
	...
	"$_ICC_COMMONS_AWS" emr terminate-clusters --cluster-ids "$_ICC_COMMONS_CLUSTER_ID" >& /dev/null || die "Failed to terminate the cluster."
}
...
function wait_cluster_running {
	...
	"$_ICC_COMMONS_AWS" emr wait cluster-running --cluster-id "$_ICC_COMMONS_CLUSTER_ID" >& /dev/null || die "Cluster timed out."
...
function wait_cluster_terminated {
	...
	"$_ICC_COMMONS_AWS" emr wait cluster-terminated --cluster-id "$_ICC_COMMONS_CLUSTER_ID" >& /dev/null || die "Cluster timed out."
...
function create_key {
	...
	"$_ICC_COMMONS_AWS" ec2 create-key-pair --key-name "$_ICC_COMMONS_KEY_NAME" --query "KeyMaterial" > "$_ICC_COMMONS_KEY_FILE" >& /dev/null || die "Failed to create EC2 key pair."
...
function delete_key {
	...
	_icc_commons_delete_key || die "Failed to delete EC2 key pair."
	rm -f "$_ICC_COMMONS_KEY_FILE"
}
...
function create_bucket {
	...
	"$_ICC_COMMONS_AWS" s3 mb "$BUCKET" >& /dev/null || die "Failed to create S3 bucket."
	_ICC_COMMONS_BUCKET_CREATED=true
}

function delete_bucket {
	log "Deleting S3 bucket"
	_icc_commons_delete_bucket || die "Failed to delete S3 bucket."
}
...
function bucket_ul {
	"$_ICC_COMMONS_AWS" s3 cp "`_icc_commons_resolve \"$1\"`" "$BUCKET/$2" >& /dev/null || die "Upload to S3 bucket failed."
}

function bucket_dl {
	"$_ICC_COMMONS_AWS" s3 cp "$BUCKET/$1" "`_icc_commons_resolve \"$2\"`" >& /dev/null || die "Download from S3 bucket failed."
}

function extract_results {
	tar xf "$RES_TEMP_TGZ" -C "$OUTPUT_DIR" --overwrite >& /dev/null || die "Extraction failed."
}
